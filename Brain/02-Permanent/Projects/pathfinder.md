...
type: permanent
source_type: project
anytype_id: bafyreignw5o3ezigv434jeag3vyksmzxvjfblstarnijcay7b5sm42v6aq
created_by: human
updated_by: claude-opus-4-6
agent_version: '02.25'
local_path: /Users/jlb/Documents/Projects/Inflection/pathfinder
status: Shelved
---
# pathfinder   
...
```
### Logging   
Logging can be configured using the `RUST\_LOG` environment variable. We recommend setting it when you invoke the run command:   
```
...
```
At the more verbose log levels (`trace`, `debug`), you may find the logs a bit noisy as our dependencies also add their own logging to the mix. You can restrict the logs to only `pathfinder` specific ones using `RUST\_LOG=pathfinder=<level>` instead. For example:   
```
